...
class RingDetector(Node):
    ...
    def remove_edge_spurs(self, canny_edges, max_steps=10):
        """Remove spurs from Canny edges by trimming from edge endpoints."""
        canny = canny_edges.copy()
        h, w = canny.shape

        # Find edge pixels
        edge_pixels = np.argwhere(canny == 255)
        edge_set = set((y, x) for y, x in edge_pixels)
        ...
        def get_neighbors(y, x):
            ...
            return [(y + dy, x + dx) for dy, dx in offsets
                    if 0 <= y + dy < h and 0 <= x + dx < w and (y + dy, x + dx) in edge_set]

        # Find endpoints (pixels with only 1 8-connected neighbor)
        endpoints = []
        for y, x in edge_pixels:
            if len(get_neighbors(y, x)) == 1:
                endpoints.append((y, x))

        # Walk from each endpoint and remove up to `max_steps` pixels
        visited = set()
        for start in endpoints:
            ...
            # Zero out pixels in the path
            for y, x in path:
                canny[y, x] = 0
                edge_set.discard((y, x))
    ...
    def image_callback(self, data):
        ...
        depth_buffer = np.nan_to_num(depth_image, nan=0.0)
        max_depth = 2.0
        depth_buffer = depth_buffer.astype(np.float32)
        depth_buffer[depth_buffer > max_depth] = max_depth
        depth_buffer = (depth_buffer / max_depth * 255.0)
        depth_buffer = np.clip(depth_buffer, 0, 255).astype(np.uint8)

        # Canny edge detection
        canny_edges = cv2.Canny(depth_buffer, 50, 150)

        
        # Harris corner detection
        gray_edges_float = np.float32(canny_edges)
        corners = cv2.cornerHarris(gray_edges_float, blockSize=5, ksize=3, k=0.1)

        corner_thresh = 0.25 * corners.max()
        corner_mask = (corners > corner_thresh).astype(np.uint8) * 255  # Binary mask

        # Dilate the corner points to create "cutting masks"
        dilation_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))  # You can increase size
        corner_mask_dilated = cv2.dilate(corner_mask, dilation_kernel, iterations=1)

        # Visualize Harris corners over depth (optional)
        corner_vis = cv2.cvtColor(depth_buffer, cv2.COLOR_GRAY2BGR)
        corner_vis[corner_mask_dilated > 0] = (0, 0, 255)
        ...
        contours, _ = cv2.findContours(canny_edges_cut, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

        filtered_contours_prep = []
        line_dist_thresh = 2.5  # pixels; increase this to make the line "thicker"
        max_inlier_ratio = 0.85
        min_contour_len = 10
        ...
        for cnt in contours:
            ...
            points = cnt[:, 0, :]  # (N, 2)
            center = np.array([x0.item(), y0.item()])

            vecs = points - center
            proj_lengths = np.dot(vecs, direction)
            projections = np.outer(proj_lengths, direction)  # projected points on the line
            closest_points = center + projections  # (N, 2)
            distances = np.linalg.norm(points - closest_points, axis=1)

            inlier_count = np.sum(distances < line_dist_thresh)
            inlier_ratio = inlier_count / len(points)

            if inlier_ratio < max_inlier_ratio:
                filtered_contours_prep.append(cnt)

        filtered_cont_vis = cv2.cvtColor(depth_buffer, cv2.COLOR_GRAY2BGR)
        cv2.drawContours(filtered_cont_vis, filtered_contours_prep, -1, (0, 255, 0), 1)

        contours = filtered_contours_prep

        #copy and draw contours for visualization
        if self.draw_visualization_windows:
            vw_contours = cv2.cvtColor(depth_buffer, cv2.COLOR_GRAY2BGR)
        ...
        if self.draw_visualization_windows:
            image_dict = {
                ...
                "Filtered Canny" : canny_edges_cut,
                #"Contours" : vw_contours,
                "Kept Contours" : filtered_cont_vis,
