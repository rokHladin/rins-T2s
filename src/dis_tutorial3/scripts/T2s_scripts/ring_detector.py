...
class RingDetector(Node):
    ...
    def camera_info_callback(self, msg):
        ...
        self.get_logger().info(
            f"Camera intrinsics received: fx={self.intrinsics['fx']}, fy={self.intrinsics['fy']}, "
            f"cx={self.intrinsics['cx']}, cy={self.intrinsics['cy']}, frame={self.depth_frame_id}"
        )
    ...
    def synced_callback(self, img_msg, depth_msg, pc_msg):
        ...
        try:
            ...
            return
        self.latest_pointcloud = pc_msg  # Always up to date for this frame
        ...
        corner_mask = (corners > corner_thresh).astype(np.uint8) * 255
        ...
        for cnt in contours:
            ...
            projections = np.outer(proj_lengths, direction)
            closest_points = center + projections
        ...
        # Mask ring visualization (just as before)
        for c in ring_candidates:
            ...
            # Optionally, add erosion to clean the mask
            mask_ring = cv2.erode(mask_ring, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)), iterations=1)
