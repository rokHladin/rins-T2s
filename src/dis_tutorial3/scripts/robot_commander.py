...
def load_face_positions():
    package_path = get_package_share_directory('dis_tutorial3')
    json_file_path = os.path.join(package_path, 'face_positions.json')
...
def main(args=None):
    ...
    face_positions = load_face_positions()
    
    #main movement
    while True:
        person_pos = face_positions[0]
        face_positions.pop(0)
        face_positions.append(person_pos)
        ...
        goal_pose.pose.position.x = person_pos['x']
        goal_pose.pose.position.y = person_pos['y']
        goal_pose.pose.orientation = rc.YawToQuaternion(0.0)

        #issue move command
        rc.goToPose(goal_pose)

        rc.info("Moving to person")
        while not rc.isTaskComplete():
            time.sleep(1)
        rc.info("Arrived to person!")
